// StarWarp — Code component template (WebGL fragment-shader background).
//
// Stars live in cells of (angle, LOG radius). A fixed cell height in log space
// is a streak that lengthens as it flies outward, so the elongation is the
// coordinate system rather than anything drawn per star.
//
...
export const STAR_WARP_COMPONENT = `'use client';

/** @label "Star Warp" */
/** @comment "Stars streaking outward at light speed." */
/** @defaultWidth 600 */
/** @defaultHeight 400 */
/** @controls {
  "colorA": {
...
const FRAG = [
  ...
  'uniform float uSpeed; uniform float uCount; uniform float uLength; uniform float uGlow;',
  'void main() {',
  '  vec2 p = (gl_FragCoord.xy - 0.5 * uRes) / uRes.y;',
  '  float r = max(length(p), 0.001);',
  '  float a = atan(p.y, p.x);',
  '  // Cells in (angle, LOG radius): a fixed cell height in log space is a',
  '  // streak that lengthens as it flies outward, which is the whole look.',
  '  vec2 sp = vec2(a / 6.28318 * uCount, log(r) * (7.0 / uLength) + uTime * uSpeed * 1.2);',
  '  vec2 i = floor(sp), f = fract(sp);',
  '  float h = hash(i);',
  '  float on = step(0.80, h);',
  '  float streak = smoothstep(0.5, 0.0, abs(f.y - 0.5))',
  '               * smoothstep(0.5, 0.12, abs(f.x - 0.5));',
  '  vec3 col = uBg + mix(uA, uB, hash(i + 7.7)) * on * streak * (0.25 + r * 2.2) * uGlow;',
  '  col += uB * exp(-r * 7.0) * 0.25;',
...
function StarWarp({
  ...
  useEffect(() => {
    ...
    const uLength = gl.getUniformLocation(prog, 'uLength');
    ...
    gl.uniform1f(uSpeed, warp);
    gl.uniform1f(uCount, count);
    gl.uniform1f(uLength, trail);
  ...
  }, [colorA, colorB, bgColor, speed, warp, count, trail, glow, isStatic]);
...
export default withResponsiveProps(StarWarp);
`;
