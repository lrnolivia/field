// Smoke — Code component template (WebGL fragment-shader background).
//
// The field scrolls DOWN through the sample point, which reads as smoke rising,
// while a second slower field drags it sideways so the column shears instead of
// climbing like a pillar. Density is thinned toward the top - smoke dissipates,
// it does not hit a ceiling.
//
...
export const SMOKE_COMPONENT = `'use client';

/** @label "Smoke" */
/** @comment "Plumes of smoke rising and shearing as they climb." */
/** @defaultWidth 600 */
/** @defaultHeight 400 */
/** @controls {
  "colorA": {
...
const FRAG = [
  ...
  'uniform float uScale; uniform float uRise; uniform float uDensity; uniform float uSwirl;',
  'void main() {',
  '  vec2 uv = gl_FragCoord.xy / uRes;',
  '  vec2 p = uv * vec2(uRes.x / uRes.y, 1.0) * uScale;',
  '  float t = uTime * 0.1;',
  '  // Rising plumes: the field scrolls DOWN through the sample point while a',
  '  // second, slower field drags it sideways, so the smoke shears as it climbs.',
  '  vec2 drift = vec2(fbm(p * 0.6 + vec2(t * 0.4, -t * uRise)) - 0.5,',
  '                    fbm(p * 0.6 + vec2(4.4, 1.1) - t * uRise) - 0.5) * uSwirl;',
  '  float f = fbm(p + drift * 1.8 + vec2(0.0, -t * uRise * 1.6));',
  '  float g = fbm(p * 2.1 + drift * 2.4 + vec2(2.0, -t * uRise * 2.4));',
  '  float d = clamp((f * 0.75 + g * 0.45 - 0.42) * uDensity * 2.4, 0.0, 1.0);',
  '  // Thin at the top: smoke dissipates, it does not hit a ceiling.',
  '  d *= smoothstep(1.15, 0.05, uv.y);',
  '  vec3 col = mix(uBg, mix(uA, uB, g), d);',
...
function Smoke({
  ...
  }, [colorA, colorB, bgColor, speed, scale, rise, density, swirl, isStatic]);
...
export default withResponsiveProps(Smoke);
`;
