// WarpTunnel — Code component template (WebGL fragment-shader background).
//
// 1/r is the depth of a tube seen down its own axis, so everything after that
// is a flat texture in (angle, depth) space and reads as 3D for free. The noise
// is sampled from the DIRECTION vector rather than from atan() - atan jumps by
// 2pi along the negative x axis and that jump shows up as a hard seam.
//
...
export const WARP_TUNNEL_COMPONENT = `'use client';

/** @label "Warp Tunnel" */
/** @comment "Flying down a textured tube toward a vanishing point." */
/** @defaultWidth 600 */
/** @defaultHeight 400 */
/** @controls {
  "colorA": {
...
const FRAG = [
  ...
  'uniform float uSpeed; uniform float uTwist; uniform float uRings; uniform float uDetail;',
  'void main() {',
  '  vec2 p = (gl_FragCoord.xy - 0.5 * uRes) / uRes.y;',
  '  float r = max(length(p), 0.02);',
  '  float a = atan(p.y, p.x);',
  '  // 1/r is the depth of a tube seen down its axis. Everything else is just a',
  '  // texture in (angle, depth) space, which is why it reads as 3D for free.',
  '  float z = 0.5 / r + uTime * uSpeed;',
  '  a += z * uTwist * 0.2;',
  '  float f = fbm(vec2(cos(a), sin(a)) * uRings * 0.35 * uDetail + z * 0.8);',
  '  float ring = 0.5 + 0.5 * sin(z * 6.0 + f * 4.0);',
  '  vec3 col = uBg + mix(uA, uB, f) * ring * smoothstep(0.0, 0.45, r);',
  '  // Black hole down the middle so the vanishing point actually vanishes.',
...
function WarpTunnel({
  ...
  useEffect(() => {
    ...
    const uRings = gl.getUniformLocation(prog, 'uRings');
  ...
  }, [colorA, colorB, bgColor, speed, flight, twist, rings, detail, isStatic]);
...
export default withResponsiveProps(WarpTunnel);
`;
