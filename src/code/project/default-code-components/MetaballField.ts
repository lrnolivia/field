// MetaballField — Code component template (WebGL fragment-shader background).
//
// An implicit surface: sum an inverse-square potential over seven moving
// centres and threshold the total. The merging comes for free from the sum -
// two blobs bulge toward each other before they touch, which is exactly what
// compositing two circles cannot do.
//
...
export const METABALL_FIELD_COMPONENT = `'use client';

/** @label "Metaball Field" */
/** @comment "Soft blobs that merge and split as they pass each other." */
/** @defaultWidth 600 */
/** @defaultHeight 400 */
/** @controls {
  "colorA": {
...
const FRAG = [
  ...
  'uniform float uSpeed; uniform float uSize; uniform float uThresh; uniform float uEdge;',
  'void main() {',
  '  vec2 p = (gl_FragCoord.xy - 0.5 * uRes) / uRes.y;',
  '  // Inverse-square potential summed over the blobs: the classic implicit',
  '  // surface, and the reason two blobs bulge toward each other before they',
  '  // touch instead of just overlapping.',
  '  float field = 0.0;',
  '  for (int i = 0; i < 7; i++) {',
  '    float fi = float(i);',
  '    vec2 c = vec2(sin(uTime * uSpeed * 0.45 + fi * 2.13) * 0.62,',
  '                  cos(uTime * uSpeed * 0.37 + fi * 1.71) * 0.40);',
  '    float r = (0.15 + 0.055 * sin(fi * 3.1)) * uSize;',
  '    vec2 d = p - c;',
  '    field += r * r / max(dot(d, d), 1e-4);',
  '  }',
  '  float m = smoothstep(uThresh - uEdge * 0.35, uThresh + uEdge * 0.35, field);',
  '  vec3 col = mix(uBg, mix(uA, uB, clamp(field / (uThresh * 2.2), 0.0, 1.0)), m);',
...
function MetaballField({
  ...
  useEffect(() => {
    ...
    const uThresh = gl.getUniformLocation(prog, 'uThresh');
    ...
    gl.uniform1f(uThresh, threshold);
  ...
  }, [colorA, colorB, bgColor, speed, drift, size, threshold, edge, isStatic]);
...
export default withResponsiveProps(MetaballField);
`;
