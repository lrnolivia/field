// Fireflies — Code component template (WebGL fragment-shader background).
//
// Each fly drifts on two incommensurate periods, so no two ever retrace the
// same path, and blinks on a phase drawn from its own cell hash. Cubing the
// blink curve is what makes it a firefly rather than a pulsing lamp: mostly
// dark, briefly bright.
//
...
export const FIREFLIES_COMPONENT = `'use client';

/** @label "Fireflies" */
/** @comment "Points of warm light drifting and blinking in the dark." */
/** @defaultWidth 600 */
/** @defaultHeight 400 */
/** @controls {
  "colorA": {
...
const FRAG = [
  ...
  'uniform float uDensity; uniform float uSpeed; uniform float uGlow; uniform float uSize;',
  'void main() {',
  '  vec2 p = (gl_FragCoord.xy - 0.5 * uRes) / uRes.y;',
  '  vec3 col = uBg + uA * 0.05 * (1.0 - dot(p, p));',
  '  vec2 g = p * uDensity;',
  ...
  '      float h = hash(id);',
  '      if (h < 0.45) continue;',
  '      // Lissajous drift - two incommensurate periods so no fly ever retraces',
  '      // its own path, and each one blinks on its own phase.',
  '      float t = uTime * uSpeed;',
  '      vec2 c = o + 0.5 + vec2(sin(t * (0.4 + h * 0.5) + h * 21.0),',
  '                              cos(t * (0.33 + h * 0.42) + h * 13.0)) * 0.34 - f;',
  '      float d = length(c);',
  '      float blink = 0.35 + 0.65 * pow(0.5 + 0.5 * sin(t * 1.8 + h * 40.0), 3.0);',
  '      float r = (0.020 + 0.016 * hash(id + 4.0)) * uSize;',
  '      col += mix(uA, uB, hash(id + 8.8))',
  '           * (smoothstep(r, 0.0, d) * 1.6 + exp(-d * (5.0 / uGlow)) * 0.5) * blink;',
  '    }',
  '  }',
...
function Fireflies({
  ...
  }, [colorA, colorB, bgColor, speed, density, drift, glow, size, isStatic]);
...
export default withResponsiveProps(Fireflies);
`;
